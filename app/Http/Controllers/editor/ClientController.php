...
class ClientController extends Controller
{
    ...
    {
        return view('pages.editor.client.index');
    }
    ...
    public function getData(Request $request): JsonResponse
    {
        ...
        try {
            $query = Client::where('title', 'LIKE', '%'.$cari.'%');
            $client = $query->offset($start)
                ->limit($limit)
                ->get();
            $client_total = $query->count();
            $data['draw'] = intval($request->input('draw'));
            $data['recordsTotal'] = $client_total;
            $data['recordsFiltered'] = $client_total;
            $data['data'] = $client;
    ...
    public function storeData(Request $request): JsonResponse
    {
        ...
        try {
            $rules = [
                'title' => 'required|string|max:255|unique:client,title',
